...
from xnkit import app
from xnkit.helpers import gen
    ...
    try:
        if app.long(m) == 1:
            ...
            m = await app.send_edit(
                m, "Finding word in this chat . . .", text_type=["mono"]
            )
            for msg in messages:
...
async def temperature_handler(_, m: Message):
    if app.long(m) <= 2:
        return await app.send_edit(
            m,
            f"How To Use: `{app.MyPrefix()[0]}temp 10 c`",
...
async def urllink_handler(app, m: Message):
    ...
    if reply:
        ...
        if msg:
            ...
            await app.send_edit(
                m, "There are no links in this message . . .", text_type=["mono"]
            )
    else:
        await app.send_edit(
            m,
...
async def forward_handler(_, m: Message):
    ...
    try:
        if reply and app.long(m) == 1:
            ...
            await app.send_edit(
                m,
                "Sir reply to yours or someone's message. to forward.",
                text_type=["mono"],
                delme=4,
            )
            delete = False
...
async def speedtest_handler(app, m: Message):
    if app.long(m) == 1:
        ...
        if teks:
            ...
            await app.send_edit(
                m, "Something went wrong !", text_type=["mono"], delme=5
            )
    ...
    try:
        ...
        if reply:
            ...
            if bool(collect):
                await app.send_edit(
                    m,
                    f"**Common chats with:** `{reply.from_user.first_name}`\n\n"
                    + "".join(collect),
                )
            else:
                await app.send_edit(
            ...
            await app.send_edit(
                m, "Please reply to someone . . .", text_type=["mono"], delme=4
            )
    except Exception as e:
        await app.error(m, e)
